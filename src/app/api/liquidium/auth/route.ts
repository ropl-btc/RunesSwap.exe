...
export async function POST(request: NextRequest) {
  ...
  const { ordinalsAddress, paymentAddress, ordinalsSignature, paymentSignature, ordinalsNonce, paymentNonce } = validation.data;
  ...
  try {
    ...
    const authSubmitResponse: LiquidiumAuthSubmitSuccessResponse = await liquidium.authSubmit(submitData);
    ...
    console.log('[API Debug] Upserting Liquidium JWT with data:', upsertData);
    ...
    if (error) {
      ...
      return createErrorResponse('Failed to store Liquidium JWT', JSON.stringify(error), 500);
    }
    console.log('[API Debug] Upsert result:', upsertResult);
