...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!address) {
      return createErrorResponse('Missing address', 'address query param is required', 400);
    }
    ...
    return createErrorResponse('Failed to fetch Liquidium portfolio', error instanceof Error ? error.message : String(error), 500);
  }
} 