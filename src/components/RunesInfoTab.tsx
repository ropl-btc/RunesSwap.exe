...
export function RunesInfoTab({
  ...
  const [runeInfoSearchQuery, setLocalRuneInfoSearchQuery] = useState(
    persistedRuneSearchQuery,
  );
  const [selectedRuneForInfo, setLocalSelectedRuneForInfo] =
  ...
  const availableRunes = runeInfoSearchQuery.trim()
    ? searchResults
    : popularRunes;
  const isLoadingRunes = runeInfoSearchQuery.trim()
    ? isSearching
    : isPopularLoading;
  const currentRunesError = runeInfoSearchQuery.trim()
    ? searchError
    : popularError;
        ...
        {!isDetailedRuneInfoLoading && !showLoading && detailedRuneInfo && (
          <div>
            ...
            {detailedRuneInfo.mint_end_block !== null && (
              <p>
                <strong>Mint End Block:</strong>{" "}
                {detailedRuneInfo.mint_end_block}
              </p>
            )}
