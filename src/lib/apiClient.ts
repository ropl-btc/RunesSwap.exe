...
export interface LiquidiumPrepareBorrowResponse {
    ...
    data?: {
        ...
        base64_psbt: string;
        sides: { // Array defining which inputs to sign
            index: number;
            address: string | null;
            sighash: number | null;
...
export const prepareLiquidiumBorrow = async (
    params: {
    ...
    if (!response.ok) {
        throw new Error(data?.error?.message || data?.error || `Failed to prepare borrow: ${response.statusText}`);
    }
    return data as LiquidiumPrepareBorrowResponse;
};

// Submit Liquidium Borrow Transaction
export const submitLiquidiumBorrow = async (
    params: {
    ...
    try {
        ...
        if (!response.ok) {
            const errorMessage = data?.error?.message || data?.error || `Failed to submit borrow: ${response.statusText}`;
            throw new Error(errorMessage);
        }
